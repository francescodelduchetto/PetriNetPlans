...
void action_cmd_callback(const std_msgs::String::ConstPtr& msg)
{
    ...
    if (actioncmd=="start" || actioncmd=="end" || actioncmd=="interrupt") {
        ...
        goal.id = hasher(actionname + "_" + actionparams);
        ...
        if (pnpac->waitForActionServerToStart(ros::Duration(1.0))) {
            // send goal and register a callback for when the action is done
            pnpac->sendGoal(goal, &action_transition_callback, &action_feedback_callback);
        }
        else
...
int main(int argc, char** argv)
{
    ...
    if (learning)
        conditionChecker = new ROSReward();
    else
        conditionChecker = new ROSConds();
